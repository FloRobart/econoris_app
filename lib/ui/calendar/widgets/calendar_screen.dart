...
class _CalendarPageState extends State<CalendarPage> {
  ...
  List<Operation> _getEventsForDay(DateTime day) =>
            ...
            ? Column(
                children: _getEventsForDay(_selectedDay ?? _focusedDay)
                    .map(
                      (op) => ListTile(
                        title: Text(op.label),
                        subtitle: Text(op.levyDate.toIso8601String()),
                        trailing: Text(op.amount.toStringAsFixed(2)),
                        onTap: () => widget.onOperationTap(op),
                      ),
                    )
                    .toList(),
              )
