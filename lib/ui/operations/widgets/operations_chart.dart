...
class OperationsChart extends StatelessWidget {
  ...
  Widget _buildLine() {
    ...
    List<MapEntry<DateTime, double>> aggregate(
      List<MapEntry<DateTime, double>> src,
      String unit,
    ) {
    ...
    Widget bottomTitleWidgets(double value, TitleMeta meta) {
      ...
      if (labels.length <= 5) {
        return Padding(
          ...
          child: Text(labels[intIndex], style: const TextStyle(fontSize: 10)),
        );
      }

      final step = (labels.length / 4).ceil();
      if (intIndex % step == 0 ||
          intIndex == labels.length - 1 ||
          intIndex == 0) {
        return Padding(
          padding: const EdgeInsets.only(top: 4),
          child: Text(labels[intIndex], style: const TextStyle(fontSize: 10)),
          ...
          touchTooltipData: LineTouchTooltipData(
            getTooltipItems: (List<LineBarSpot> touchedSpots) {
              return touchedSpots.map((spot) {
                ...
                final y = spot.y;
                final dateStr = entries.length > 360
                    ? DateFormat('MM/yyyy').format(dt)
                    : DateFormat('yyyy-MM-dd').format(dt);
                return LineTooltipItem(
                  '$dateStr\n${currencyFmt.format(y)}',
