...
class SubscriptionsTable extends StatelessWidget {
  ...
  const SubscriptionsTable({
    ...
    this.maxItems,
    this.columns = const ['start_date', 'name', 'amount', 'active'],
    this.onRowTap,
  });
  ...
  List<DataCell> _buildCells(Subscription s) {
    ...
    return columns.map((c) {
      switch (c) {
        case 'start_date':
          return DataCell(
            Text(df.format(s.startDate)),
            onTap: () => onRowTap?.call(s),
          );
        case 'name':
          return DataCell(Text(s.label), onTap: () => onRowTap?.call(s));
        case 'amount':
          Color? amountColor = s.amount > 0
              ? Colors.green
              : (s.amount < 0 ? Colors.red : null);
          return DataCell(
            Text(
        ...
        case 'interval':
          return DataCell(
            Text('${s.intervalValue} ${s.intervalUnit}'),
            onTap: () => onRowTap?.call(s),
          );
        case 'id':
          return DataCell(
  ...
  Widget build(BuildContext context) {
    final items = (maxItems != null)
        ? subscriptions.take(maxItems!).toList()
        : subscriptions;
    return DataTable(
      columns: _buildColumns(context),
      rows: items.map((s) => DataRow(cells: _buildCells(s))).toList(),
    );
  }
}
