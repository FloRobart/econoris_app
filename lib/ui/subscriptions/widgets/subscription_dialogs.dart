...
class _SubscriptionDetailDialogState extends State<SubscriptionDetailDialog> {
  Future<void> _delete() async {
    ...
    if (resp.statusCode >= 200 && resp.statusCode < 300) {
      ...
      try {
        GlobalData.instance.removeSubscriptionById(widget.subscription.id);
      } catch (_) {}
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      actions: [
        ...
        TextButton(
          onPressed: () async {
            final ok = await AddOperationFab.showSubscriptionEditor(
              context,
...
class SubscriptionCreateDialog extends StatefulWidget {
  ...
  @override
  State<SubscriptionCreateDialog> createState() =>
      _SubscriptionCreateDialogState();
}
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: TextFormField(
                      ...
                      onChanged: (v) => _intervalValue = int.tryParse(v) ?? 1,
                    ),
                  ),
                  const SizedBox(width: 8),
      ...
      actions: [
        ...
        TextButton(onPressed: () => _save(), child: const Text('Créer')),
      ],
    );
  }
