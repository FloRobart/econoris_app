...
class _SubscriptionsPageState extends State<SubscriptionsPage> {
  ...
  void _openDetail(Subscription s) async {
    final result = await showDialog<String>(
        context: context,
        builder: (_) => SubscriptionDetailDialog(subscription: s));
  ...
  Widget build(BuildContext context) {
    final subsFiltered = _subscriptions.where((sub) {
      if (_categoryFilter != 'Tous' && sub.category != _categoryFilter) {
        return false;
      }
      ...
      if (_search.isNotEmpty) {
        ...
        return sub.label.toLowerCase().contains(s) ||
            (sub.source ?? '').toLowerCase().contains(s) ||
            (sub.destination ?? '').toLowerCase().contains(s);
      }
      return true;
    }).toList();
    ...
    subsFiltered.sort((a, b) {
      ...
      switch (_sortField) {
        ...
        case 'operations_name':
          ...
          break;
        case 'start_date':
          res = a.startDate.compareTo(b.startDate);
          break;
        case 'operations_id':
    ...
    final double expenseCurrent = ops.where((s) => s.amount < 0).fold(
        0.0, (double acc, Subscription s) => acc + monthlyEquivalent(s).abs());
    ...
    if (revenueCurrent == 0) {
      ...
      revenueToShow = _subscriptions
          .where((s) =>
              s.startDate.year == displayRevenueYear &&
              s.startDate.month == displayRevenueMonth &&
              s.amount > 0)
          .fold(
              0.0, (double acc, Subscription s) => acc + monthlyEquivalent(s));
    }
    ...
    final categories =
        ['Tous'] + _subscriptions.map((e) => e.category).toSet().toList();
                      ...
                      children: [
                            ...
                            child: Column(
                                ...
                                children: [
                                  Row(children: [
                                    ...
                                    const Spacer(),
                                    Text('${ops.length} abonnements',
                                        style: theme.textTheme.bodyMedium),
                                  ]),
                                  const SizedBox(height: 8),
                                  SizedBox(
                                      height: 220,
                                      child: SubscriptionsChart(
                                          subscriptions: ops,
                        ...
                        Row(children: [
                          ...
                          PopupMenuButton<String>(
                            ...
                            itemBuilder: (_) => const [
                              PopupMenuItem(
                                  value: 'start_date', child: Text('Date')),
                              PopupMenuItem(
                                  value: 'operations_name', child: Text('Nom')),
                              PopupMenuItem(
                                  value: 'operations_amount',
                                  child: Text('Montant')),
                              PopupMenuItem(
                                  value: 'operations_id', child: Text('ID')),
                            ],
                          ),
                          IconButton(
                              icon: Icon(_sortAsc
                            ...
                            child: Column(
                                children: pageItems
                                    .map((s) => ListTile(
                                          title: Text(s.label),
                                          subtitle: Text(
                                              '${s.startDate.toIso8601String()} • ${s.category}'),
                                          trailing:
