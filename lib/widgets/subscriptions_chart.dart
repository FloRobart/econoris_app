...
class SubscriptionsChart extends StatelessWidget {
  ...
  const SubscriptionsChart({super.key, required this.subscriptions, required this.chartType});

  @override
  Widget build(BuildContext context) {
    if (subscriptions.isEmpty) return const Center(child: Text('Pas d\'abonnements'));
  ...
  Widget _buildLine() {
    ...
    var entries = grouped.entries.map((e) => MapEntry(DateTime.parse(e.key), e.value)).toList()..sort((a,b)=>a.key.compareTo(b.key));
    ...
    Widget bottomTitleWidgets(double value, TitleMeta meta) {
      ...
      return Padding(padding: const EdgeInsets.only(top:4), child: Text(labels[idx], style: const TextStyle(fontSize:10)));
    }
    ...
    final lineChart = LineChart(LineChartData(
      ...
      lineBarsData: [LineChartBarData(isCurved:false, spots:spots, dotData: const FlDotData(show:true))],
      minX: 0,
      maxX: spots.isNotEmpty ? spots.last.x : 0,
    ));

    return Padding(padding: const EdgeInsets.only(top:8,bottom:4,left:4,right:32), child: SizedBox(height:220, child: lineChart));
  }

  Widget _buildBar(){
    final sorted = List<Subscription>.from(subscriptions)..sort((a,b)=>a.startDate.compareTo(b.startDate));
  ...
  Widget _buildPie(){
    ...
    int i=0;
    map.forEach((k,v){ sections.add(PieChartSectionData(value:v, title:k, radius:40 + (i*2))); i++; });
