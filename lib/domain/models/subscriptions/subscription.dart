class Subscription {
  ...
  factory Subscription.fromJson(Map<String, dynamic> j) {
    ...
    return Subscription(
      ...
      intervalValue: (j['interval_value'] is int)
          ? j['interval_value']
          : int.tryParse(j['interval_value']?.toString() ?? '') ?? 1,
      intervalUnit: j['interval_unit'] ?? 'months',
      startDate: parseDate(j['start_date']),
      endDate: j['end_date'] == null ? null : parseDate(j['end_date']),
      dayOfMonth: j['day_of_month'] == null
          ? null
          : (j['day_of_month'] is int
                ? j['day_of_month']
                : int.tryParse(j['day_of_month']?.toString() ?? '')),
      lastGeneratedAt: j['last_generated_at'] == null
          ? null
          : parseDate(j['last_generated_at']),
